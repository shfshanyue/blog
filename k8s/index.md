...
在 `master` 节点执行以下命令，获取需要预先下载好的镜像列表
...
在 `proxy` 节点，把镜像列表存为 `images.txt`，并批量下载镜像与打包

> 此时，我也把 kuberneted-dashboard 与 metrics-server 的镜像也一并放在 images.txt 中，稍后有用
